...
public class PartD {
	...
	public static ArrayList<String> main(int totalNodes, int noOfHospitalNodes, int[] hospitalArray,
			ArrayList<ArrayList<String>> aL_Original) {
		...
		while (true) {
			...
			for (int i = 0; i < aL_Original.size(); i++) {
				ArrayList<String> arrayListClone = (ArrayList<String>) aL_Original.get(i).clone();
				aL.add(arrayListClone);
			}

			System.out.println("Please enter k: (Enter -1 to exit)");
			int k = sc.nextInt();
			if (k == -1) {
				break;
			} else if (k > noOfHospitalNodes) {
				System.out.println("K has exceeded the total number of hospital nodes: " + noOfHospitalNodes);
				continue;
			}
			...
			if (hospitalList.stream().anyMatch(j -> j == i)) {
			...
			for (int a = 0; a < k; a++) {
				String out = print(i, hospitalList, totalNodes, aL, a + 1);
	...
	private static String print(int sNode, ArrayList<Integer> hospitalList, int totalNodes,
			ArrayList<ArrayList<String>> al, int k) {
	...
	private static int breadthFirstSearch(int sNode, int[] tStore, int[] dStore, int totalNodes, ArrayList<ArrayList<String>> al) {
