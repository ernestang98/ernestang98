...
def run(weights,  # model.pt path(s)
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            p = Path(p)  # to Path
            save_path = increment_path(save_dir / p.name, exist_ok=exist_ok)  # img.data
            print('')
            print(save_path)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        label = None if hide_labels else (names[c] if hide_conf else f'{names[c]},id: ' f'{label_dict.get(names[c])}')
                        annotator.box_label(xyxy, label, color=colors(c, True) )
                        if save_crop:
                            save_one_box(xyxy, imc, file=save_dir / 'crops' / names[c] / f'{p.stem}.jpg', BGR=True)


                    # Only for checklist A5, comment out bullseye and stop
                    # if names[c] == "Bullseye":
                    #     detected = True
                    ...
                    #     detected = True
                    #     sendImage(detected, dataset, save_img, save_path, im0, save_dir, save_txt, t0)
                    #     print("It is a 'stop sign'!")
                    #     return "IMG|CAR|CA5_STOP"
                    #
                    # To send to RPi
                    if names[c] != "Bullseye":
                        detected = True
                        sendImage(detected, dataset, save_img, save_path, im0, save_dir, save_txt, t0)
                        return sendRpiMsg.conversion(names[c])
...
def sendImage(detected, dataset, save_img, save_path, im0, save_dir, save_txt, t0):
